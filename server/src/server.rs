...
pub async fn spawn_client(addr1: &str, addr2: &str) -> Result<ClientHandle, io::Error> {
    ...
    tokio::spawn(async move {
        loop {
            let (socket, peer_addr) = match listener_img.accept().await {
                Ok(pair) => pair,
                Err(_) => continue,
            };
            let _ = socket_img_sender.send(ClientEvent::Connected(peer_addr)).await;

            let (mut rd, _wr) = io::split(socket);
                ...
                tokio::select! {
                    read_result = rd.read(&mut buf) => {
                        match read_result {
                            Ok(0) | Err(_) => break,
                            Ok(n) => {
                                let _ = socket_img_sender.send(ClientEvent::Data(buf[..n].to_vec())).await;
                            }
                        }
                    }
                }
